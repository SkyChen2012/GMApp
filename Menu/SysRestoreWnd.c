...
SysRestoreWndPaint(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	if (bConfirmAgain) 
	{
		if (SET_CHINESE == g_SysConfig.LangSel) 
		{
			...
			MXDrawText_Left(Hdc, "确认吗?", xPos, 0);
			MXDrawText_Left(Hdc, "按#确认", xPos, 1);
			MXDrawText_Left(Hdc, "按其他键退出", xPos, 2);
		}
	...
	else
	{
		...
		{
			xPos = GetXPos("按其它键返回");
			MXDrawText_Left(Hdc, "按#系统还原", xPos, 1);
			MXDrawText_Left(Hdc, "按其它键返回", xPos, 2);
		}
		else if (SET_ENGLISH == g_SysConfig.LangSel)
		{
			xPos = GetXPos("Other Keys to Return");

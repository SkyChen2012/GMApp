...
#ifdef SSL_AUTH_INIT_DEBUG
  printf("ssl_flags = 0x%x\n", soap->ssl_flags); 
#endif
...
//	RSA *rsa = RSA_generate_key(SOAP_SSL_RSA_BITS, RSA_F4, NULL, NULL);
	...
	RSA* rsa = RSA_new();
	BIGNUM *bne = BN_new();
	if(!bne || !rsa || !BN_set_word(bne, RSA_F4))
	{
#ifdef SSL_AUTH_INIT_DEBUG
		printf("RSA_generate_key_ex: Memory error...\n");
#endif
		return soap_set_receiver_error(soap, "SSL/TLS error", "RSA_generate_key_ex: Memory error", SOAP_SSL_ERROR);
	}
	ret = RSA_generate_key_ex(rsa, SOAP_SSL_RSA_BITS, bne, NULL);
	if(!ret) {
#ifdef SSL_AUTH_INIT_DEBUG
		...
		if (rsa)
			RSA_free(rsa);		
		return soap_set_receiver_error(soap, "SSL/TLS error", "RSA_generate_key_ex error", SOAP_SSL_ERROR);
	}
...
#else
/*#if defined(LIB_OS_IOS)
    soap_set_test_logfile(soap, "/private/var/mobile/Applications/05AC5608-D8C6-4C9B-A750-A19B458D4D31/tmp/TEST.log");
    soap_set_sent_logfile(soap, "/private/var/mobile/Applications/05AC5608-D8C6-4C9B-A750-A19B458D4D31/tmp/SENT.log");
    soap_set_recv_logfile(soap, "/private/var/mobile/Applications/05AC5608-D8C6-4C9B-A750-A19B458D4D31/tmp/RECV.log");
#else*/
